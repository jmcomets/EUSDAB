#ifndef STATES_SHIELDBREAK_H
#define STATES_SHIELDBREAK_H

#include <state.h>

namespace EUSDAB
{
    namespace States
    {
        class ShieldBreak: public State
        {
            public:
                ShieldBreak(ShieldBreak &&) = default;
                ShieldBreak(const ShieldBreak &) = delete;
                ShieldBreak & operator=(const ShieldBreak &) = delete;

                ShieldBreak(Movement const &);
                virtual ~ShieldBreak();

                void onUp(const Event &);
                void onDown(const Event &);
                void onLeft(const Event &);
                void onRight(const Event &);

                void onA(const Event &);
                void onB(const Event &);
                //void onX(const Event &);
                //void onY(const Event &);
                //void onZ(const Event &);
                //void onTrigger(const Event &);

                void onNextFrame();

                //void onAttack(const Event &);
                //void onDamage(const Event &);

                //void onGround(const Event &);

                void onEnter();
                void onLeave();
                
                void onAnimationEnd();

                
        };
    }
}

#endif


